...
const TabList = ({ experiences }: Props) => {
  ...
  const { role, company, companyUrl, client, clientUrl, started, upto, tasks } =
  ...
  return (
    <div className="flex flex-col sm:flex-row text-sm md:text-base gap-6 md:gap-10 min-h-[250px]">
      {/* Sidebar */}
      <div className="font-mono text-xs sm:text-sm relative flex justify-start sm:flex-col overflow-scroll sm:overflow-auto sm:min-w-[260px] sm:max-w-[260px]">
        {experiences.map(({ company, client }, i) => (
          <button
            key={getId()}
            title={client ? `${company} (Client: ${client})` : company}
            className={`h-14 min-w-[120px] sm:w-[260px] sm:px-5 sm:!text-left border-b border-dark-3 last:border-b-0 hover:bg-accent-light hover:text-accent focus:outline-none focus:bg-accent-light focus:text-accent ${
          ...
          >
            ...
            {client && (
              <span className="block text-[10px] text-dark-2">
                Client: {client}
              </span>
            )}
          </button>
      ...
      <div key={getId()} className="p-1 space-y-5">
        <div className="space-y-1">
          <h3 className="text-lg font-medium capitalize text-dark-2">
            ...
            {client && clientUrl && (
              <>
                {' '}|{' '}
                <Link href={clientUrl} target="_blank" className="text-accent">
                  @{client}
                </Link>
              </>
            )}
        ...
        <div className="h-px w-full bg-dark-3" />
